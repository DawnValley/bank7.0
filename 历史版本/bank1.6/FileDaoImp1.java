  ...
  public class FileDaoImp1 implements BankDaoInterface
  {
	  ...
	  */
	  public boolean register(UserBean user) throws FileNotFoundException,IOException,Exception
	  {
		  ...
		  else if("".equals(name)||"".equals(pwd))
		  {
			  System.out.println("用户名和密码不能为空，请重新注册！");
			  return false;
		  }
		  else//此前以下语句块没有放在if{}else{}的大括号里，一旦重复注册将无法登录原账号
		  {
	  ...
	  */
	  ...
	  {
		  Properties prop2 = new Properties();//这里如果用prop打开对方文件，会出现自己文件的信息错误的情况
		  File f = new File(name+".properties");
		  if(f.exists()==true)//先要判断要转入的账号是否存在
		  {
			  ...
			  String proMoney = Double.parseDouble(prop2.getProperty("money"))+money+"";
			  prop2.setProperty("money",proMoney);
			  prop2.store(out,".\\"+name+".properties");
			  out.close();
		      return true;
		  }
		  else
