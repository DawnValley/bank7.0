...
import com.cx.bank.dao.*;
  ...
  */
  public class UserDaoFactory
  {
	  ...
	  private String surl = Thread.currentThread().getContextClassLoader().getResource("../").toString().substring(6)+"properties";
	  //purl = purl.repaceAll("%20",' ');
	  private String purl = URLDecoder.decode(surl,"UTF-8");
	  private UserDaoFactory() throws Exception//私有的构造方法，单例第二要素
	  {
		  System.out.println("路径："+purl);
		  Properties prop = new Properties();//文件对象
		  FileInputStream fis = new FileInputStream(new File(purl+"/classInfo.properties"));//文件输入流对象
