...
<script language="javascript">
		...
		if (money < 0) {
			...
			if (window.confirm('当前用户金额小于零，是否激活？')) {
				if (window.confirm('确定激活？')) {
					window.self.location = "AdministratorController.do?method=stateOperation&id=" + id
    ...
    /*首页*/
	function topPage() {
		window.self.location = "AdministratorController.do?method=userManagementInfo&pageNo=${pageMode.topPageNo}&pageSize=${pageMode.pageSize}&queryStr=${queryStr}";
	}
    /*上一页*/
	function previousPage() {
		window.self.location = "AdministratorController.do?method=userManagementInfo&pageNo=${pageMode.previousPageNo}&pageSize=${pageMode.pageSize}&queryStr=${queryStr}";
	}
    /*下一页*/
	function nextPage() {
		window.self.location = "AdministratorController.do?method=userManagementInfo&pageNo=${pageMode.nextPageNo}&pageSize=${pageMode.pageSize}&queryStr=${queryStr}";
	}
    /*尾页*/
	function bottomPage() {
		window.self.location = "AdministratorController.do?method=userManagementInfo&pageNo=${pageMode.bottomPageNo}&pageSize=${pageMode.pageSize}&queryStr=${queryStr}";
	}
</script>
</head>
...
<center>
<div class="all">
	<form action="AdministratorController.do?method=userManagementInfo&pageNo=1&pageSize=${pageMode.pageSize}" name="queryForm" method="post">
		<div class="div1">	
				...
				<c:forEach items="${pageMode.list}" var="ub">
					<tr>
						...
						<c:if test="${ub.conditions eq 'activation'}">
						  <%-- <a href="AdministratorController.do?method=stateOperation&id=${ub.id }&operation=freeze&pageNo=${pageMode.pageNo}&pageSize=${pageMode.pageSize}&querStr=${queryStr}"> --%>
						...
						<%-- <a href="AdministratorController.do?method=stateOperation&id=${ub.id }&operation=activation&pageNo=${pageMode.pageNo}&pageSize=${pageMode.pageSize}&querStr=${queryStr}">
